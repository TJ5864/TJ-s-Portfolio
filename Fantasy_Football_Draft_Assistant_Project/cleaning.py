...
try:
    df = pd.read_csv("all_positions_2024_projections.csv", header=[0, 1])
    df.columns = [' '.join(col).strip() for col in df.columns]
except:
    df = pd.read_csv("all_positions_2024_projections.csv")
...
df.to_csv("cleaned_player_projections.csv", index=False)
